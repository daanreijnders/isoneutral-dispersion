...
outFile = f"trelease_coarsened_tave_locb_{AD_kernel}_{K_kernel}_C{C}_p{nparts}_dt{int(dt//60)}m.nc"
...
def initializeParticles(reference_ds, level, nparts=100_000):
    """
    ...
    if Le_Sommer == True:
        for var in ['Delta', 'P', 'Q', 'R']:
            ...
            for derivdim in ['X', 'Y', 'Z']:
                ...
                dimensions[f"d{var.lower()}d{derivdim}"] = dict(zip(["depth", "lat", "lon"], ds[f"d{var.lower()}d{derivdim}"].dims))
