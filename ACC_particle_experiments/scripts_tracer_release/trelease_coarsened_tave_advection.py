...
outFile = f"trelease_coarsened_tave_locb_{AD_kernel}_{K_kernel}_K{kappa}_p{nparts}_dt{int(dt//60)}m.nc"
...
def initializeParticles(reference_ds, level, nparts=nparts):
    """
...
def create_fieldset(inpath, dataFile, derivatives= False, Le_Sommer=False):
    ...
    if Le_Sommer == True:
        for var in ['Delta', 'P', 'Q', 'R']:
            ...
            dimensions[var] = dict(zip(["depth", "lat", "lon"], ds[var.lower()].dims))
            for derivdim in ['X', 'Y', 'Z']:
                filenames[f"d{var}d{derivdim}"] = inpath + dataFile
                variables[f"d{var}d{derivdim}"] = f"d{var.lower()}d{derivdim}"
                dimensions[f"d{var}d{derivdim}"] = dict(zip(["depth", "lat", "lon"], ds[f"d{var.lower()}d{derivdim}"].dims))
    
    fieldset = parcels.FieldSet.from_c_grid_dataset(filenames, variables, dimensions, mesh="flat", tracer_interp_method='linear', allow_time_extrapolation=True, gridindexingtype='mitgcm', deferred_load=False)
    ...
    fieldset.add_constant("max_dt", dt)
    fieldset.add_constant("min_dt", dt/(2**4))
    return fieldset
...
pset.execute(SampleTHETAKernel, dt=0)
