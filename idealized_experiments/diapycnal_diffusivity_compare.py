...
runtypes = ["redi_full", "redi_smallslope", "redi_full_M1", "markov-1"]
diapycnal_diffusivities = pd.DataFrame(columns=runtypes, index=dt_mins)

for run in runtypes:
    if run == "redi_full":
        fieldset = create_analytical_fieldset(myField, Ki=1000, epsilon=0)
        add_dummy_settings(fieldset)
    elif run == "redi_smallslope":
        fieldset = create_analytical_fieldset(myField, Ki=1000, epsilon=0)
        add_dummy_settings(fieldset)
    elif run == "redi_full_M1":
        fieldset = create_analytical_fieldset(myField, Ki=1000, expansion_terms=8, epsilon=1e-13)
        add_dummy_settings(fieldset)
    for dt in dt_mins:
        if run == "markov-1":
            fieldset = create_analytical_fieldset(myField, Ki=1000, Tl = 20 * 24 * 60 * 60, epsilon=int(dt) * 60 / (20 * 24 * 60 * 60), eta=1e-8)
            add_dummy_settings(fieldset)
        print(f"Running {run} at dt={dt} minutes")
        testParticles = get_test_particles_dense(myField, fieldset, rho=1027.5)
        if run == "redi_full":
            ParcelsRandom.seed(1636)
            testParticles.execute(testParticles.Kernel(elements_analytical.elements_analytical) \
            ...
            testParticles.execute(testParticles.Kernel(elements_analytical.elements_analytical) \
                                + testParticles.Kernel(K_Redi.K_Redi)\
                                + testParticles.Kernel(M1_3D_BC.M1_3D_BC),
                                  runtime=timedelta(days=90),
                                  dt=timedelta(minutes=int(dt)),
                                  verbose_progress=True,
            )              
        ...
        dia_error = diapycnal_flux(testParticles.lon, testParticles.lat, testParticles.depth, 90*24*60*60)
        diapycnal_diffusivities[run][int(dt)] = dia_error
        print(dia_error)
        
diapycnal_diffusivities.to_csv("diapycnal_diffusivities_90d.csv")